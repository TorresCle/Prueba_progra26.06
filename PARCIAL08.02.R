...
# PREGUNTA N?1  -------------- Trabajaremos con la Cuenca Tumbes
################################################################
...
# Los datos extraidos mediante el filtro al conjunto de datos para la cuenta Tumbes y los escenarios
# observados nos muestran que la data corresponde a un solo a?o, sin datos faltantes ni missing values.
# Por ende se precide de realizar la comprobacion de la cantidad de datos y existencia de valores NA.

# La precipitaci?n observada anual acumulada para la cuenca Tumbes es de 852mm, ello nos indica
# que se trata de una zona con precipitacion alta.

#SEGUNDO METODO - FIORELLA BOCANEGRA
cuenca01 <- read_csv("mods_clima_uh.csv")
cuenca01 %>% 
  filter(uh_name == "Cuenca Tumbes") %>%                       
  group_by(bh_esc) %>% 
  summarise(bh_pc = sum(bh_pc, na.rm = T)) %>%#para saber solo de la categoria observado se ppuede hacer filter de nuevo 
...
grafico <- read_csv("mods_clima_uh.csv") %>%
  ...
  mutate(bh_month = sprintf("%1$02d", bh_month)) # Convertidos los meses a numeros de dos d?gitos


table(grafico$bh_esc) #Se saca un resumen de lo que se tiene
view(grafico)


nombres <- c("ACCESS 1.0", "HadGEM2-ES", "MPI-ESM-LR", "Observado")
meses <- data.frame(month.abb)

for (i in 1:4) {
  data01 <- grafico %>% 
    filter(bh_esc == nombres[i]) %>% 
    select(bh_pc)
  meses <- cbind(meses, data01)
  print(resumen)
}
...
modelo <- c("ACCESS 1.0", "HadGEM2-ES", "MPI-ESM-LR", "Observado")
...
for (x in 1:4) {
  a <- grafico %>%
    filter(bh_esc == modelo[x]) %>%
    select(bh_pc) # %>%rename(oso = bh_pc)
  tabla1 <- cbind(tabla, a)
  print(tabla)
}

colnames(tabla1) <- c("meses","ACCESS 1.0", "HadGEM2-ES", "MPI-ESM-LR", "Observado")


for (i in 1:4){
  valor <- grafico %>% 
    filter(bh_esc == modelos[i]) %>% 
    select(bh_pc) %>% 
  resumen <- cbind(tabla, valor)
}
...
# Convertidos los meses a numeros de dos d?gitos


table(grafico$bh_esc)
# Mediante este filtro observamos que tenemos 3 modelos mas la precipitaci?n observada en nuestra cuenca.
# cada una de ella con un a?o de dato generados.

ggplot(data = grafico) +
  geom_bar(mapping = aes(x = bh_month, y = bh_pc)) +
  scale_x_discrete(
    labels = month.abb
  ) +
...
ggplot(grafico) +
  geom_bar(stat = "identity",mapping =  aes(x = bh_month, y = bh_pc)) +
  facet_wrap(~bh_esc, nrow = 2)
...
# PREGUNTA N?1  -------------- Travbajaremos con la estaci?n qc00000746
#######################################################################
data <-
...
cantidad_na <- function(a, b) {
  ...
  data %>%
    ...
    mutate(
      cantidad_NA = sum(is.na(tmp)) # indicr? nos numeros de NA en un nuevo campo llamado missval(en porcentaje). la funcion n me arroja el numero de dias.
    ) %>%
    summarise(
...
(temp_mensual <-
  ...
  mutate(
    valores_NA = sum(is.na(tmp)) * 100 / n() # indicr? nos numeros de NA en un nuevo campo llamado missval(en porcentaje). la funcion n me arroja el numero de dias.
  ) %>%
  summarise( # Tabla resumen
    tmp = mean(tmp, na.rm = T), # realizamos la tmp acumulada
    valores_NA = unique(valores_NA), # Genera un unico valor, la funcion unique me devuelve valores ?nicos
  ) %>%
  mutate(
...
ggplot(temp_mensual, aes(DATE, tmp)) +
  ...
  labs(x = "A?os analizados", y = "Temperatura") +
...
#En un solo gr?fico de l?neas.

# Para esta situaci?n vemos que los valores promedios de temperatura mensual en ambos casos no var?a mucho
# distribuyendose en la mayor?a de casos entre valores de 28 y 25 grados.
...
# entre valores de 25 a 29 grados, as? mismo se observan datos que no coinciden con la tendencia como
# puntos que sobresalen de los gr?ficos de cajas. La mayor varianza se observa en el mes de febrero.
